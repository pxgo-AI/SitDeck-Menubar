...
> SitDeck Menubar  — A lightweight, zero-dependency utility layer for async state management in modern JavaScript applications.
...
```javascript
const { useAsyncState } = require('SitDeck Menubar');
...
```javascript
const { init } = require('SitDeck Menubar');
...
MIT © Copyright (c) 2026 SitDeck Menubar, pxgo.
